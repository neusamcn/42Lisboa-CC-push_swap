...
starting by making a right shift that places the provided octet's 1st bit in the last bit's position
and ending with the last bit in the provided octet's original position.

    octet & 1 + '0'
To be able to use write() to return this unsigned CHAR octet in binary (only chars '0' and '1'), it's necessary to
transform the representation of the octet from a SINGLE UNSIGNED CHAR to EIGHT CHAR THAT CAN ONLY BE '0' OR '1'.
How to solve this:
...
the provided octet's LSB (Least Significant Bit = rightmost bit) will be revealed,
since the resulting octet of this operation can only be either
