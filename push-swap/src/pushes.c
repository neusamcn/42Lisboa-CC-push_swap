...
int	pop_stack_head(t_stack *stack, t_circlist *pop_node)
{
	...
	t_circlist *old_head;
	t_circlist *new_head;
	t_circlist *head_previous;

	if (!stack || !stack->head || stack->head != pop_node || stack->size < 1)
		return (-1);
	old_head = stack->head;
	new_head = stack->head->next;
	head_previous = stack->head->previous;
	if (pop_node == old_head && (stack->head->next == stack->head || stack->size == 1))
	{
	...
	else if (pop_node == old_head)
	{
		...
		new_head->previous = old_head->previous;
